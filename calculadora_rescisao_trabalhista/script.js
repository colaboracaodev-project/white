...
//     document.getElementById('resultado').innerHTML += "<br> Total: R$ " + Number(total) + "<br>";
// }
...
form.addEventListener('submit', function (e) {
    ...
    const diasMes = parseInt(document.getElementById('dias-trabalhados').value);
    const mesesAno = parseInt(document.getElementById('meses-trabalhados-ano').value);
    const mesesTrabalhados = parseInt(document.getElementById('total-de-meses-trabalhados').value);
...
// funções auxiliares
function calculaSaldoSalario(salarioDigitado, diasMes) {
    saldoSalario = (salarioDigitado / 30) * diasMes;
    return saldoSalario;
}

function decimoTerceiroProporcional(salarioDigitado, mesesAno) {
    decimoTerceiro = (salarioDigitado / 12) * mesesAno;
    return decimoTerceiroProporcional;
}

function calculaFeriasVencidas(salarioDigitado) {
    feriasVencidas = salarioDigitado + (salarioDigitado * (1 / 3));
    return feriasVencidas;
}

function calculaFeriasProporcionais(mesesAno, feriasVencidas) {
    feriasProporcionais = (feriasVencidas / 12) * mesesAno;
    return feriasProporcionais;
}
